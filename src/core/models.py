...
class InlineButton(BaseModel, frozen=True):
...
class OutboundMessage(BaseModel, frozen=True):
    ...
    inline_keyboard: list[list[InlineButton]] | None = Field(
        default=None,
        description="Inline keyboard buttons (rows of buttons)",
    )
...
class TimezoneSource(str, Enum):
    ...
    RELOCATION_CONFIRMED = "relocation_confirmed"  # User confirmed relocation city
...
class SessionGoal(str, Enum):
    ...
    CLARIFY_GEO_INTENT = (
        "clarify_geo_intent"  # City detected, agent decides: time query or relocation
    )
...
class PipelineResult(BaseModel):
    ...
    Contains triggers, context, and any messages generated by handlers.
    Orchestrator uses triggers + context to decide if session creation is needed.
    """

    # All detected triggers (for orchestrator decision making)
    triggers: list[DetectedTrigger] = Field(default_factory=list)
    # Resolved context (timezone, target timezones, etc.)
    context: ResolvedContext | None = Field(default=None)
    # Messages generated by handlers (empty if handlers couldn't handle)
    messages: list[OutboundMessage] = Field(default_factory=list)
    # Errors during detection/handling
